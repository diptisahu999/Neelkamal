...
class AccountMove(models.Model):
    ...
    def compute_stock_type(self):
        for rec in self:
            ...
            if self.name != '/':
                rec.picking_count = len(self.env['stock.picking'].search(['|',('id', '=', rec.invoice_picking_id.id),('origin', '=', rec.name)]))
            else:
                rec.picking_count = 0
        ...
        for order in self:
            # if not self.invoice_picking_id:
                ...
                order = order.invoice_line_ids.filtered(lambda item:
                                                        ...
                                                        ['product', 'consu'])
                (order._create_stock_moves(picking)._action_confirm()._action_assign())
                picking.button_validate()

    def action_view_picking(self):
        """Function to view moves while clicking shipment smart button"""
        action = self.sudo().env.ref('stock.action_picking_tree_ready')
        result = action.read()[0]
        result.pop('id', None)
        result['context'] = {}
        result['domain'] = ['|',('id', '=', self.invoice_picking_id.id),('origin', '=', self.name)]

        pick_ids = self.env['stock.picking'].sudo().search(['|',('id', '=', self.invoice_picking_id.id),('origin', '=', self.name)])

        # pick_ids = sum([self.invoice_picking_id.id])
        if len(pick_ids) == 1:
            res = self.sudo().env.ref('stock.view_picking_form', False)
            result['views'] = [(res and res.id or False, 'form')]
            result['res_id'] = pick_ids.id or False
        else:
            return {
